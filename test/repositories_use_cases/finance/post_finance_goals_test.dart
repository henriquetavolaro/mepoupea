import 'package:dartz/dartz.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:mepoupeapp/data/repositories_use_cases_impl/finance/post_finance_goals_impl.dart';
import 'package:mepoupeapp/domain/model/finance_goal_input.dart';
import 'package:mockito/mockito.dart';

import '../onboarding/get_onboard_answer_test.mocks.dart';

main() {
  final service = MockNetworkService();

  final useCase = PostFinanceGoalsImpl(service);

  final financeGoalInput = FinanceGoalInput(
      actualValue: 0.0,
      goalValue: 0.0,
      valueToSave: 0.0,
      goalType: "goalType",
      importance: "importance",
      dateRange: "dateRange",
      icon: "icon");

  test('should return Right int - code 200', () async {
    when(service.postFinanceGoals(financeGoalInput)).thenAnswer((_) async => 200);

    final result = await useCase(financeGoalInput);

    expect(result.fold(id, id), isA<int>());
  });

  test('should return Left Exception', () async {
    when(service.postFinanceGoals(financeGoalInput))
        .thenThrow((_) async => Exception());

    final result = await useCase(financeGoalInput);

    expect(result.fold(id, id), isA<Exception>());
  });
}
